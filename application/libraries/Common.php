...
class Common {
	...
	function __construct() {
		...
		/* URL */
		$project_folder = "the-learning-guide";
		$allowed_domains = array('bhimani.com.au');
		$domain  = $_SERVER['HTTP_HOST'];
		$request_scheme = "http";
		$current = (isset($_SERVER['PATH_INFO'])) ? $_SERVER['PATH_INFO'] : '';

		$notArr = array('/Home/get_notify','/find-teacher','/find-school','/assets/user/css/bootstrap.css.map');
		if(!in_array($current, $notArr)) {
			...
			$request_scheme = "http";
			$url = $request_scheme."://".$domain."/".$project_folder.$current;
			/* URL */
			$check = $this->get_one_row('tbl_traffic',array('ip'=>$ip,'url'=>$url) );
			if($check == '') {

				$pageViewData = array('ip'=>$ip,'url'=>$url);
				$insertPageView = $this->insert_record('tbl_traffic',$pageViewData);
			}
		}
		/* PAGE VIEW */
	}
	...
	public function get_all($tbl,$wh = array(),$params = array(),$joins='')
	{
	...
	public function get_all_join_record($tables,$joins,$rows="*",$groupBy="",$order_by=array(),$wh = array(),$keywords='',$field_array=array(),$params = array(),$find_in_set='')
	{
        ...
        if(array_key_exists("where_or",$params)){
            $this->CI->db->or_where($params['where_or']);
        }
    ...
    // Join Data
	public function search_query($tables,$rows="*",$wh = array(),$keywords='',$field_array=array())
	{
		$this->CI->db->select($rows);
	...
	public function get_num_rows_with_where($table_name,$where = array()) {
		if (!empty($where)) {
