...
class Configuration extends CI_Controller {
    ...
    public function update_setting()
    {
        ...
        if($this->Settingmodel->batch_update('tblsetting','SettingKey',$config)){
            if ($_FILES['Admin_Logo_2']['error'] == 0) {
                $ext = pathinfo($_FILES['Admin_Logo_2']['name'], PATHINFO_EXTENSION);
                $result = $this->general->upload_file("Admin_Logo_2", UPLOADPATHADMIN,'admin_logo_2.'.$ext, "jpg|png|jpeg");
                if (is_array($result)) {
                    $this->session->flashdata('error','But Sorry, Admin Logo Not Uploaded.!!!');
                    redirect(base_url(ADMIN.'Configuration/setting'));
                }
                $update[] = array("SettingKey" => "Admin_Logo_2","SettingValue" =>'admin_logo_2.'.$ext);
                $this->Settingmodel->batch_update('tblsetting','SettingKey',$update);
            }
            if ($_FILES['FrontEnd_Logo_2']['error'] == 0) {
                $ext = pathinfo($_FILES['FrontEnd_Logo_2']['name'], PATHINFO_EXTENSION);
                $result = $this->general->upload_file("FrontEnd_Logo_2", UPLOADPATHFRONT,'logo.'.$ext, "jpg|png|jpeg");
                if (is_array($result)) {
                    $this->session->flashdata('error','But Sorry, Front End Logo Not Uploaded.!!!');
                    redirect(base_url(ADMIN.'Configuration/setting'));
                }
                $update[] = array("SettingKey" => "FrontEnd_Logo_2","SettingValue" =>'logo.'.$ext);
                $this->Settingmodel->batch_update('tblsetting','SettingKey',$update);
            }
